...
class Pause extends Command {
  ...
  async run(message, args) {
    ...
    if(!message.guild.voiceConnection.player.dispatcher) {
      return client.utils.get("music").sendEmbed(message, ":point_up::skin-tone-3: Je n'ai pas de musique à jouer !");
    }
      if(message.guild.voiceConnection.player.dispatcher.paused) {
        await message.guild.voiceConnection.player.dispatcher.resume();
        await client.utils.get("music").sendEmbed(message, "▶ La musique était déjà en pause, j'ai donc remis la musique sur play");
      } else {
        await message.guild.voiceConnection.player.dispatcher.pause();
        await client.utils.get("music").sendEmbed(message, "⏸ J\'ai mis votre musique en pause");
      }
  }
}
